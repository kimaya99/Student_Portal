...
public class DropWaitListActivity extends AppCompatActivity implements AdapterView.OnItemClickListener{

    Button dropCourse;
    String redId, password,firstName,lastName,email,courseId;
    ListView waitListCoursesList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_drop_waitlist);

        dropCourse = (Button) findViewById(R.id.dropWaitButton);
        back = (ImageButton) findViewById(R.id.imageButton3);
        waitListCoursesList = (ListView) findViewById(R.id.WaitDropList);
    ...
    private void addToDisplayView(List<String> list) {
        for(int k=0; k<list.size(); k++){
            dataAdapter = new ArrayAdapter<>(DropWaitListActivity.this, android.R.layout.simple_spinner_item,list);
            dataAdapter.setDropDownViewResource(android.R.layout.simple_list_item_1);
            waitListCoursesList.setAdapter(dataAdapter);
            waitListCoursesList.setOnItemClickListener(this);
        }
    }
    ...
    private void unRegisterStudent() {
        ...
        String url = "https://bismarck.sdsu.edu/registration/unwaitlistclass";
        JsonObjectRequest postRequest = new JsonObjectRequest(url, DropWaitListActivity.this.jsonData, success, failure) ;
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        String selectedOpt = (String) waitListCoursesList.getItemAtPosition(position);
        ...
        for (int i =0;i< listForTitle.size();i++){
            if (selectedOpt.equals(listForTitle.get(i))){
                ...
                waitListCoursesList.getChildAt(i).setBackgroundColor(Color.LTGRAY);
            }
        }
